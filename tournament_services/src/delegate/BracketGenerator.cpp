...
void BracketGenerator::GenerateWinnersBracket(
    ...
    // Round 2: 8 matches (W16-W23) - empty
    for (int i = 16; i < 24; ++i) {
        domain::Match match;
        match.Name() = "W" + std::to_string(i);
        match.TournamentId() = tournamentId;
    ...
    // Round 3: 4 matches (W24-W27) - empty
    for (int i = 24; i < 28; ++i) {
        domain::Match match;
        match.Name() = "W" + std::to_string(i);
        match.TournamentId() = tournamentId;
    ...
    // Round 4: 2 matches (W28-W29) - empty
    for (int i = 28; i < 30; ++i) {
        domain::Match match;
        match.Name() = "W" + std::to_string(i);
        match.TournamentId() = tournamentId;
    ...
    // Round 5: 1 match (W30) - Winners Bracket Final - empty
...
void BracketGenerator::GenerateLosersBracket(
    ...
    // Losers Round 1: 8 matches (L0-L7) - empty
    for (int i = 0; i < 8; ++i) {
        domain::Match match;
    ...
    // Losers Round 2: 8 matches (L8-L15) - empty
    for (int i = 8; i < 16; ++i) {
        domain::Match match;
        match.Name() = "L" + std::to_string(i);
        match.TournamentId() = tournamentId;
    ...
    // Losers Round 3: 4 matches (L16-L19) - empty
    for (int i = 16; i < 20; ++i) {
        domain::Match match;
        match.Name() = "L" + std::to_string(i);
        match.TournamentId() = tournamentId;
    ...
    // Losers Round 4: 4 matches (L20-L23) - empty
    for (int i = 20; i < 24; ++i) {
        domain::Match match;
        match.Name() = "L" + std::to_string(i);
        match.TournamentId() = tournamentId;
    ...
    // Losers Round 5: 2 matches (L24-L25) - empty
    for (int i = 24; i < 26; ++i) {
        domain::Match match;
        match.Name() = "L" + std::to_string(i);
        match.TournamentId() = tournamentId;
    ...
    // Losers Round 6: 2 matches (L26-L27) - empty
    for (int i = 26; i < 28; ++i) {
        domain::Match match;
        match.Name() = "L" + std::to_string(i);
        match.TournamentId() = tournamentId;
