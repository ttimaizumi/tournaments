// TournamentControllerTest.cpp -  requimientos minimos

#include <gtest/gtest.h>
#include <gmock/gmock.h>
#include <crow.h>
#include <nlohmann/json.hpp>

#include "controller/TournamentController.hpp"
#include "delegate/ITournamentDelegate.hpp"
#include "domain/Tournament.hpp"

using ::testing::StrictMock;
using ::testing::Return;
using ::testing::DoAll;
using ::testing::SaveArg;
using ::testing::Truly;
using ::testing::_;
using ::testing::Eq;

// Mock del delegate con todos los metodos que usa el controller
class MockTournamentDelegate : public ITournamentDelegate {
public:
    MOCK_METHOD(std::string, CreateTournament, (std::shared_ptr<domain::Tournament>), (override));
    MOCK_METHOD(std::vector<std::shared_ptr<domain::Tournament>>, ReadAll, (), (override));
    MOCK_METHOD(std::shared_ptr<domain::Tournament>, ReadById, (std::string_view), (override));
    MOCK_METHOD(bool, UpdateTournament, (const domain::Tournament&), (override));
};

struct TournamentControllerFixture : ::testing::Test {
    std::shared_ptr<StrictMock<MockTournamentDelegate>> delegateMock;
    std::shared_ptr<TournamentController> controller;

    void SetUp() override {
        delegateMock = std::make_shared<StrictMock<MockTournamentDelegate>>();
        controller   = std::make_shared<TournamentController>(delegateMock);
    }
};

/* =====================================================================================
 * REQUERIMIENTO: Create -> validar transformacion JSON -> dominio y valor transferido
 * al TournamentDelegate. Respuesta HTTP 201.
 * - Se captura el objeto enviado al delegate y se validan sus campos.
 * - Se valida HTTP 201 y header Location.
 * ===================================================================================== */
TEST_F(TournamentControllerFixture, CreateTournament_201_LocationHeader_And_TransfersDomainValues) {
    nlohmann::json reqBody = { {"id","t1"}, {"name","Summer Cup"} };
    crow::request req; req.body = reqBody.dump();

    std::shared_ptr<domain::Tournament> captured;
    EXPECT_CALL(*delegateMock, CreateTournament(_))
        .WillOnce(DoAll(SaveArg<0>(&captured), Return(std::string{"t1"})));

    crow::response res = controller->CreateTournament(req);

    EXPECT_EQ(res.code, crow::CREATED);
    ASSERT_NE(captured, nullptr);
    EXPECT_EQ(captured->Id(),   "t1");
    EXPECT_EQ(captured->Name(), "Summer Cup");
    // opcional pero util: validar Location
    EXPECT_EQ(res.get_header_value("location"), "t1");
}

/* =====================================================================================
 * Extra (no requerido, robustez): Create -> JSON invalido -> HTTP 400
 * ===================================================================================== */
TEST_F(TournamentControllerFixture, CreateTournament_400_BadJson) {
    crow::request req; req.body = "{ not-json ";
    auto res = controller->CreateTournament(req);
    EXPECT_EQ(res.code, crow::BAD_REQUEST);
}

/* =====================================================================================
 * REQUERIMIENTO: Create -> validar transformacion JSON -> dominio y valor transferido
 * al TournamentDelegate. Simular error de insercion en BD. Respuesta HTTP 409.
 * - Se captura el objeto enviado al delegate y se validan sus campos.
 * - Se valida HTTP 409.
 * ===================================================================================== */
TEST_F(TournamentControllerFixture, CreateTournament_409_Conflict_TransfersDomainValues) {
    nlohmann::json body = { {"id","dup"}, {"name","X"} };
    crow::request req; req.body = body.dump();

    std::shared_ptr<domain::Tournament> captured;
    EXPECT_CALL(*delegateMock, CreateTournament(_))
        .WillOnce(DoAll(
            SaveArg<0>(&captured),
            Return(std::string{}) // simulamos fallo
        ));

    auto res = controller->CreateTournament(req);
    EXPECT_EQ(res.code, crow::CONFLICT);

    // validar objeto transferido al delegate
    ASSERT_NE(captured, nullptr);
    EXPECT_EQ(captured->Id(),   "dup");
    EXPECT_EQ(captured->Name(), "X");
}

/* =====================================================================================
 * REQUERIMIENTO: ReadById -> validar que el valor (id) transferido al
 * TournamentDelegate es el esperado. Simular objeto y validar HTTP 200.
 * ===================================================================================== */
TEST_F(TournamentControllerFixture, ReadById_200_Found_TransfersIdToDelegate) {
    auto t = std::make_shared<domain::Tournament>();
    t->Id() = "t9";
    t->Name() = "Gamma";

    EXPECT_CALL(*delegateMock, ReadById(Eq(std::string_view{"t9"})))
        .WillOnce(Return(t));

    auto res = controller->ReadById("t9");
    EXPECT_EQ(res.code, crow::OK);

    auto json = nlohmann::json::parse(res.body);
    EXPECT_EQ(json.at("id").get<std::string>(), "t9");
    EXPECT_EQ(json.at("name").get<std::string>(), "Gamma");
}

/* =====================================================================================
 * REQUERIMIENTO: ReadById -> validar que el valor (id) transferido al
 * TournamentDelegate es el esperado. Simular nulo y validar HTTP 404.
 * ===================================================================================== */
TEST_F(TournamentControllerFixture, ReadById_404_NotFound_TransfersIdToDelegate) {
    EXPECT_CALL(*delegateMock, ReadById(Eq(std::string_view{"zzz"})))
        .WillOnce(Return(nullptr));

    auto res = controller->ReadById("zzz");
    EXPECT_EQ(res.code, crow::NOT_FOUND);
}

/* =====================================================================================
 * REQUERIMIENTO: ReadAll -> simular resultado con lista de objetos. HTTP 200.
 * ===================================================================================== */
TEST_F(TournamentControllerFixture, ReadAll_200_WithItems) {
    auto t1 = std::make_shared<domain::Tournament>();
    t1->Id() = "t1";
    t1->Name() = "Alpha";

    auto t2 = std::make_shared<domain::Tournament>();
    t2->Id() = "t2";
    t2->Name() = "Beta";

    EXPECT_CALL(*delegateMock, ReadAll())
        .WillOnce(Return(std::vector<std::shared_ptr<domain::Tournament>>{t1, t2}));

    crow::response res = controller->ReadAll();
    EXPECT_EQ(res.code, crow::OK);

    auto json = nlohmann::json::parse(res.body);
    ASSERT_TRUE(json.is_array());
    ASSERT_EQ(json.size(), 2);
    EXPECT_EQ(json[0].at("id").get<std::string>(), "t1");
    EXPECT_EQ(json[0].at("name").get<std::string>(), "Alpha");
    EXPECT_EQ(json[1].at("id").get<std::string>(), "t2");
    EXPECT_EQ(json[1].at("name").get<std::string>(), "Beta");
}

/* =====================================================================================
 * REQUERIMIENTO: ReadAll -> simular resultado con lista vacia. HTTP 200.
 * ===================================================================================== */
TEST_F(TournamentControllerFixture, ReadAll_200_EmptyList) {
    EXPECT_CALL(*delegateMock, ReadAll())
        .WillOnce(Return(std::vector<std::shared_ptr<domain::Tournament>>{}));

    crow::response res = controller->ReadAll();
    EXPECT_EQ(res.code, crow::OK);

    auto json = nlohmann::json::parse(res.body);
    ASSERT_TRUE(json.is_array());
    EXPECT_TRUE(json.empty());
}

/* =====================================================================================
 * REQUERIMIENTO: Update -> validar transformacion JSON -> dominio y valor
 * transferido al TournamentDelegate. HTTP 204.
 * - Controller hace ReadById(routeId), aplica cambios de JSON al dominio, luego llama Update.
 * - Se valida que el objeto pasado a Update tenga Id de ruta y el nuevo nombre.
 * ===================================================================================== */
TEST_F(TournamentControllerFixture, UpdateTournament_204_NoContent_TransfersDomainValues) {
    nlohmann::json body = { {"id","DIFF"}, {"name","New Name"} };
    crow::request req; req.body = body.dump();

    auto existing = std::make_shared<domain::Tournament>();
    existing->Id() = "t1";
    existing->Name() = "Old Name";

    EXPECT_CALL(*delegateMock, ReadById(Eq(std::string_view{"t1"})))
        .WillOnce(Return(existing));

    EXPECT_CALL(*delegateMock,
        UpdateTournament(Truly([](const domain::Tournament& tt){
            return tt.Id() == "t1" && tt.Name() == "New Name";
        })))
        .WillOnce(Return(true));

    auto res = controller->UpdateTournament(req, "t1");
    EXPECT_EQ(res.code, crow::NO_CONTENT);
}

/* =====================================================================================
 * REQUERIMIENTO: Update -> validar transformacion JSON -> dominio y valor
 * transferido al TournamentDelegate. Simular ID no encontrado y validar HTTP 404.
 * - En esta ruta, el controller solo alcanza a llamar ReadById(routeId). Si no existe,
 *   no llama Update. Validamos que ReadById reciba el id esperado y que el status sea 404.
 * ===================================================================================== */
TEST_F(TournamentControllerFixture, UpdateTournament_404_NotFound_ReadByIdTransfersRouteId) {
    nlohmann::json body = { {"id","missing"}, {"name","X"} };
    crow::request req; req.body = body.dump();

    EXPECT_CALL(*delegateMock, ReadById(Eq(std::string_view{"missing"})))
        .WillOnce(Return(nullptr));
    // StrictMock garantiza que UpdateTournament NO se llame, pero lo hacemos explicito:
    EXPECT_CALL(*delegateMock, UpdateTournament(_)).Times(0);

    auto res = controller->UpdateTournament(req, "missing");
    EXPECT_EQ(res.code, crow::NOT_FOUND);
}

/* =====================================================================================
 * Extra (no requerido, robustez): ReadById -> id con formato invalido -> HTTP 400
 * ===================================================================================== */
TEST_F(TournamentControllerFixture, ReadById_400_BadIdFormat) {
    auto res = controller->ReadById("bad id!!"); // no pasa regex
    EXPECT_EQ(res.code, crow::BAD_REQUEST);
}

/* =====================================================================================
 * Extra (no requerido, robustez): Update -> JSON invalido -> HTTP 400
 * ===================================================================================== */
TEST_F(TournamentControllerFixture, UpdateTournament_400_BadJson) {
    crow::request req; req.body = "not json";
    auto res = controller->UpdateTournament(req, "anyid");
    EXPECT_EQ(res.code, crow::BAD_REQUEST);
}
